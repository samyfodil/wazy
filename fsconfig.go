...
// # Windows
//
// While wazy supports Windows as a platform, all known compilers use POSIX
...
func (c *fsConfig) WithSysFSMount(fs sys.FS, guestPath string) FSConfig {
	...
	cleaned := internalsys.StripPrefixesAndTrailingSlash(guestPath)
	ret := c.clone()
