...
same function via reflection (`WithFunc`) **1086 ns/op, 6 allocs** (14.5x).

- **H1. Reflect path has no fast path** — `internal/wasm/gofunc.go:84-140`. Per call: `make([]reflect.Value, pLen)`, `reflect.New(t).Elem()` per param, `newContextVal`/`newModuleVal` (2 allocs each; `reflect.ValueOf` would do), `fn.Call` allocates results. Fix: at `parseGoReflectFunc` time, type-switch against common shapes (ctx/mod + up to ~4 i32/i64 params) and emit direct `api.GoModuleFunction` adapters; precompute type tables for the fallback. Impact: high, **measured**, certain.
  - **DONE**: 378 generated adapters (`gofunc_fast.go`, generator `gen_gofunc_fast.go`) + precomputed fallback tables. Measured: 1106→102 ns/op (Call, 8→2 allocs), 1062→81 ns/op (CallWithStack, 6→0 allocs) — parity with direct `GoModuleFunc` within ~5 ns. Equivalence + zero-alloc guard tests in `gofunc_fast_test.go`.
  - Follow-up idea: a typed generic registration API (e.g. `WithTypedFunc(func(ctx, mod, uint32) uint32)`) would skip the registration-time type-switch and the residual ~4 ns per-call `Interface().(T)` assertion. Generics cannot replace the reflect fallback itself (Go can't abstract over func arity), so this is additive public API, not a rewrite.
- **H2. itab hash lookup per host call** — `wazevo/hostmodule.go:65-76` + `call_engine.go:388-439`. `readIface` + `.(api.GoModuleFunction)` → `runtime.assertE2I`/`getitab` every call (~8.4% cum). Fix: assert once in `buildHostModuleOpaque`, store the asserted iface words. Impact: medium, certain.
- **H3. Trampoline saves 23 regs one at a time (arm64)** — `arm64/abi_go_call.go` — 46 memory instrs per host call. Fix: `stp`/`ldp` pairing (q-regs already at 16-byte stride); pack int regs at 8-byte stride. amd64: 26 movs, same idea. Impact: medium, certain.
...
- **H5. Non-inlinable closure + conditional defer wraps every host call** — `call_engine.go:389-394` etc. (~2%). Fix: branch to a separate func with defer only when snapshotter enabled. Impact: low-med, certain.
...
- **R2. All decoding via `bytes.Reader` byte-at-a-time** + `nextByte` closures (leb128 takes `io.ByteReader`); in-code TODO acknowledges it. Fix: `(buf, offset)` decoder + slice-based LEB; typical 1.5-3x decode win. Impact: high, certain.
...
- **R4. `decodeCode`: body copied per function + locals parsed twice** (`binary/code.go:30-114`). Fix: one arena for the code section; single-pass locals. Impact: med-high, certain.
...
- **A7. Snapshotter `ctx.Value` on every call** — see I4/H7. Fix: process-level `atomic.Bool` latch + per-activation caching. Impact: medium, certain.
