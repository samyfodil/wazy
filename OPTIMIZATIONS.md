...
- **C4. No const-fold/GVN/CSE/copy-prop SSA passes** — **RESOLVED**: `passConstFoldAndCSE` adds integer constant folding (wasm wrap/shift-mask exact), algebraic identities (x+0, x*1, x*0, x&0, ...), and per-block CSE over a pure-opcode whitelist, before DCE. -61% on redundancy-heavy hand-written wasm; parity (no regression) on LLVM-produced code (which already folds), so it won't move the vs-wazero Execute gap but helps naive producers and is groundwork for later passes. Compile-time neutral (O(1) opcode prefilter). Correctness: all 14 backend fusion-assumption sites audited; only the 2 `lowerExitIfTrueWithCode` trap-Icmp sites are sharp (both guarded by a complete pre-scan); CSE's RefCount interaction is self-protecting elsewhere. Verified on both ISAs incl. arm64 spectest under qemu.
