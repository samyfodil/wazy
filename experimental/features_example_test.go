...
func ExampleCoreFeaturesThreads() {
	...
	cfg := wazy.NewRuntimeConfig().WithCoreFeatures(api.CoreFeaturesV2 | experimental.CoreFeaturesThreads)
	...
	if _, err := r.InstantiateWithConfig(ctx, memoryWasm, wazy.NewModuleConfig().WithName("env")); err != nil {
		log.Panicln(err)
	}
...
func createChildModule(rt wazy.Runtime, root api.Module, wasmCompiled wazy.CompiledModule) *childModule {
	childModuleMu.Lock()
	...
	child, err := rt.InstantiateModule(ctx, wasmCompiled, wazy.NewModuleConfig().
...
func ExampleCoreFeaturesTailCall() {
	...
	cfg := wazy.NewRuntimeConfig().WithCoreFeatures(api.CoreFeaturesV2 | experimental.CoreFeaturesTailCall)
