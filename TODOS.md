...
## Delete the dead instantiateWithImports path (cleanup, no behavior change)
- **What:** single-core host-import components are now routed through the graph engine (`needsImportPath`→`instantiateGraph`), so `instantiateWithImports` is unreachable from `Instantiate` -- dead code, still exercised only by its own direct tests.
- **Why:** the graph engine's anonymous-internals model made multi-component composition work (see below); `instantiateWithImports` registered internals under raw global names and can't be fixed the same way (the ImportResolver can't carry host modules). Now superseded.
- **Context:** delete `instantiateWithImports` + its exclusive helpers (`moduleNameFor`, `bindImportExports`, `bindFuncExport`, `bindInstanceExport`, `resolveCoreFuncCanon`) + the ~33 test references in `host_import_test.go`/`bench_test.go`. `buildHostWrapper` is SHARED with the graph path (`buildCanonHostModule`) -- keep it. Verify the full fixture + 35/35 conformance suite after.

## DONE: multi-component composition (wasmtime model)
- A Runtime now hosts any number of component instances (distinct AND multiple of the same), and one component can call another on one Runtime (`TestOneComponentCallsAnotherOnOneRuntime`). The graph engine instantiates internals anonymously (`WithName("")`) and wires them via a per-instantiation `experimental.ImportResolver`, so nothing internal touches the global registry -- like wasmtime. Compile cache intact (empty-import rewrite → stable `graphEmptyImportKey`). See the [[wasip2-component-model]] memory for the full mechanism.
