...
func TestBuilder_passConstFoldAndCSE_exitIfTrueGuard(t *testing.T) {
	...
	t.Run("const-operand Icmp feeding ExitIfTrueWithCode is NOT folded", func(t *testing.T) {
		...
		b.AllocateInstruction().AsExitIfTrueWithCode(execCtx, cmp, wazevoapi.ExitCodeIntegerDivisionByZero).Insert(b)
	...
	t.Run("a NON-guarded const Icmp is still folded even beside a guarded one", func(t *testing.T) {
		...
		b.AllocateInstruction().AsExitIfTrueWithCode(execCtx, guardedCmp, wazevoapi.ExitCodeIntegerDivisionByZero).Insert(b)
	...
	t.Run("two identical Icmps each feeding ExitIfTrueWithCode are NOT CSE-merged", func(t *testing.T) {
		...
		b.AllocateInstruction().AsExitIfTrueWithCode(execCtx, cmp1, wazevoapi.ExitCodeMemoryOutOfBounds).Insert(b)
		cmp2 := b.AllocateInstruction().AsIcmp(x, y, IntegerCmpCondUnsignedLessThan).Insert(b).Return()
		b.AllocateInstruction().AsExitIfTrueWithCode(execCtx, cmp2, wazevoapi.ExitCodeMemoryOutOfBounds).Insert(b)
	...
	t.Run("operands of a guarded Icmp are still constant-folded", func(t *testing.T) {
		...
		b.AllocateInstruction().AsExitIfTrueWithCode(execCtx, cmp, wazevoapi.ExitCodeMemoryOutOfBounds).Insert(b)
