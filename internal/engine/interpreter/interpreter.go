...
func (e *moduleEngine) MemoryGrown(wasm.Index) {}
...
type callEngine struct {
	...
	//
	// This is a value slice (not []*callFrame): pushFrame/popFrame append/
	// truncate by value so pushing a frame no longer heap-allocates. Any
	// *callFrame taken from this slice (e.g. via &ce.frames[i]) is only valid
	// until the next operation that can append to ce.frames, since that
	// append may grow and relocate the backing array.
	frames []callFrame
...
func (e *moduleEngine) ResolveImportedMemory(index, indexInImportedModule wasm.Index, importedModuleEngine wasm.ModuleEngine) {
}
...
func (ce *callEngine) callNativeFunc(ctx context.Context, m *wasm.ModuleInstance, f *function) {
	...
	// (Call/CallIndirect/CallRef/ReturnCallRef/tail-call-to-other-module) can
	// itself push frames, so frame is re-taken from ce.frames after each one.
	frame := &ce.frames[len(ce.frames)-1]
	body := frame.f.parent.body
	// I3: pc is a local (register) mirror of frame.pc so the hot dispatch loop
	// reads/advances a value instead of touching heap memory every instruction.
	// The loop guard indexes directly against len(body) (rather than a separate
	// bodyLen variable) so the compiler proves body[pc] is in bounds and drops
	// the per-instruction bounds check: a separate length variable would be an
	// independent phi across the loop back-edge that the prover can't tie back to
	// len(body).
	//
	// Sync policy: pc is authoritative between operations; frame.pc is only
	// guaranteed current immediately after a sync. We store pc back to frame.pc
	// ("frame.pc = pc") before any operation that can be observed via frame.pc:
	// (1) ops that trap/panic (unreachable, memory load/store, div/rem, trunc
	// float->int, ref.as_non_null, and the rare-op group) so a fault reports the
	// faulting instruction's pc exactly as before; (2) calls into wasm (so a
	// snapshot clone, an exception-table search in an unwinding callee, and a
	// stack trace all see this frame's pc); (3) throw/throw_ref (searchExceptionTable
	// reads frame.pc). We reload pc from frame.pc after anything that changes it
	// (exception handler application, tail-call resetPc, snapshot restore). The
	...
	for pc < uint64(len(body)) {
		...
		// len(Buffer)" check below, because len(mem.Buffer) can never
		...
		case operationKindLoadI32, operationKindLoadF32:
			...
			ce.pushValue(uint64(binary.LittleEndian.Uint32(mem.Buffer[ea:])))
			pc++
		case operationKindLoadI64, operationKindLoadF64:
			...
			ce.pushValue(binary.LittleEndian.Uint64(mem.Buffer[ea:]))
			pc++
		case operationKindLoad8S32:
			...
			ce.pushValue(uint64(uint32(int8(mem.Buffer[ea]))))
			pc++
		case operationKindLoad8S64:
			...
			ce.pushValue(uint64(int8(mem.Buffer[ea])))
			pc++
		case operationKindLoad8U32, operationKindLoad8U64:
			...
			ce.pushValue(uint64(mem.Buffer[ea]))
			pc++
		case operationKindLoad16S32:
			...
			ce.pushValue(uint64(uint32(int16(binary.LittleEndian.Uint16(mem.Buffer[ea:])))))
			pc++
		case operationKindLoad16S64:
			...
			ce.pushValue(uint64(int16(binary.LittleEndian.Uint16(mem.Buffer[ea:]))))
			pc++
		case operationKindLoad16U32, operationKindLoad16U64:
			...
			ce.pushValue(uint64(binary.LittleEndian.Uint16(mem.Buffer[ea:])))
			pc++
		case operationKindLoad32S:
			...
			ce.pushValue(uint64(int32(binary.LittleEndian.Uint32(mem.Buffer[ea:]))))
			pc++
		...
		case operationKindStore16:
			...
			binary.LittleEndian.PutUint16(mem.Buffer[ea:], val)
			pc++
...
func (ce *callEngine) callNativeFuncRare(op *unionOperation, frame *callFrame, functions []function, memories []*wasm.MemoryInstance, tables []*wasm.TableInstance, dataInstances []wasm.DataInstance, elementInstances []wasm.ElementInstance) {
	...
	case operationKindMemoryCopy:
		...
		if rangeOutOfBounds(sourceOffset, copySize, srcLen) ||
			...
			copy(dstMem.Buffer[destinationOffset:],
				srcMem.Buffer[sourceOffset:sourceOffset+copySize])
		}
	case operationKindMemoryFill:
		...
		if rangeOutOfBounds(offset, fillSize, uint64(len(mem.Buffer))) {
			...
			buf := mem.Buffer[offset : offset+fillSize]
			if value == 0 {
				clear(buf)
			} else {
				buf[0] = value
	...
	case operationKindAtomicFence:
		// Fence has no memory operand and isn't tied to any particular memory; any mutex's
		// Lock/Unlock provides the same CPU-level barrier, so any one memory's Mux suffices.
		if len(memories) > 0 {
			...
			memories[0].Mux.Lock()
			memories[0].Mux.Unlock() //nolint:staticcheck
		}
...
func (ce *callEngine) resetPc(frame *callFrame, f *function) (body []unionOperation) {
