		...
		},
		{
			...
			m: &wasm.ModuleInstance{
				Globals: []*wasm.GlobalInstance{
					{
						Me: &moduleEngine{
							parent: &compiledModule{offsets: nativeapi.ModuleContextOffsetData{GlobalsBegin: importedGlobalBegin}},
...
func TestModuleEngine_ResolveImportedFunction(t *testing.T) {
	...
	for i, tc := range []struct {
		...
		t.Run(strconv.Itoa(i), func(t *testing.T) {
			buf := m.opaque[begin+nativeapi.FunctionInstanceSize*tc.index:]
...
func Test_functionInstance_offsets(t *testing.T) {
	...
	require.Equal(t, nativeapi.FunctionInstanceSize, int(unsafe.Sizeof(fi)))
	require.Equal(t, nativeapi.FunctionInstanceExecutableOffset, int(unsafe.Offsetof(fi.executable)))
	require.Equal(t, nativeapi.FunctionInstanceModuleContextOpaquePtrOffset, int(unsafe.Offsetof(fi.moduleContextOpaquePtr)))
	require.Equal(t, nativeapi.FunctionInstanceTypeIDOffset, int(unsafe.Offsetof(fi.typeID)))

	m := nativeapi.ModuleContextOffsetData{ImportedFunctionsBegin: 100}
	ptr, moduleCtx, typeID := m.ImportedFunctionOffset(10)
	require.Equal(t, 100+10*nativeapi.FunctionInstanceSize, int(ptr))
