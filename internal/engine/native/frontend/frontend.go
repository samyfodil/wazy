...
type Compiler struct {
	...
	varLengthKnownSafeBoundWithIDPool nativeapi.VarLengthPool[knownSafeBoundWithID]
...
type (
	...
	knownSafeBoundsAtTheEndOfBlock = nativeapi.VarLength[knownSafeBoundWithID]
)

var knownSafeBoundsAtTheEndOfBlockNil = nativeapi.NewNilVarLength[knownSafeBoundWithID]()

// NewFrontendCompiler returns a frontend Compiler.
func NewFrontendCompiler(m *wasm.Module, ssaBuilder ssa.Builder, offset *nativeapi.ModuleContextOffsetData, ensureTermination bool, listenerOn bool, sourceInfo bool) *Compiler {
	c := &Compiler{
		m:                                 m,
...
type tryTableMetadata interface {
	Append(info nativeapi.TryTableInfo) int
	Table() []nativeapi.TryTableInfo
}
...
func (t *localTryTableMetadata) Append(info nativeapi.TryTableInfo) int {
	id := len(t.table)
	t.table = append(t.table, info)
	return id
}

func (t *localTryTableMetadata) Table() []nativeapi.TryTableInfo {
	return t.table
}
...
func (s *SharedTryTableMetadata) Append(info nativeapi.TryTableInfo) int {
	if s.finalized {
...
func (s *SharedTryTableMetadata) Table() []nativeapi.TryTableInfo {
	s.finalized = true
	return s.table
}
...
func (c *Compiler) TryTableMetadata() []nativeapi.TryTableInfo {
...
// nativeapi.EhEntry values (one per contiguous run of the body's blocks in
// the final code layout -- see nativeapi.EhEntry's doc comment) using
...
type EhPendingEntry struct {
	...
	// path look up NumLocals/ReuseLocals (nativeapi.TryTableInfo) for
...
func (c *Compiler) declareSignatures(listenerOn bool) {
	...
	c.gcSig = ssa.Signature{
		ID: c.tryTableLeaveSig.ID + 1,
		// exec context, then the mode and five operands of wasm.RunGC.
		Params: []ssa.Type{
...
func (c *Compiler) LowerToSSA() {
	...
	//
	//  1. executionContextPtr: pointer to the *executionContext in native package.
	//    This will be used to exit the execution in the face of trap, plus used for host function calls.
	//
	// 	2. moduleContextPtr: pointer to the *moduleContextOpaque in native package.
	...
	//  is wazy-specific since other runtimes can naturally use the OS-level signal to do this job thanks to the fact that
