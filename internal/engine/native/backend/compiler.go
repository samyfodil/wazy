...
// Compiler is the backend of native which takes ssa.Builder and Machine,
...
func (c *compiler) Compile(ctx context.Context) ([]byte, []RelocationInfo, error) {
	c.Lower()
	if nativeapi.PrintSSAToBackendIRLowering && nativeapi.PrintEnabledIndex(ctx) {
		fmt.Printf("[[[after lowering for %s ]]]%s\n", nativeapi.GetCurrentFunctionName(ctx), c.Format())
	}
	if nativeapi.DeterministicCompilationVerifierEnabled {
		nativeapi.VerifyOrSetDeterministicCompilationContextValue(ctx, "After lowering to ISA specific IR", c.Format())
	}
	c.RegAlloc()
	if nativeapi.PrintRegisterAllocated && nativeapi.PrintEnabledIndex(ctx) {
		fmt.Printf("[[[after regalloc for %s]]]%s\n", nativeapi.GetCurrentFunctionName(ctx), c.Format())
	}
	if nativeapi.DeterministicCompilationVerifierEnabled {
		nativeapi.VerifyOrSetDeterministicCompilationContextValue(ctx, "After Register Allocation", c.Format())
	}
	if err := c.Finalize(ctx); err != nil {
		return nil, nil, err
	}
	if nativeapi.PrintFinalizedMachineCode && nativeapi.PrintEnabledIndex(ctx) {
		fmt.Printf("[[[after finalize for %s]]]%s\n", nativeapi.GetCurrentFunctionName(ctx), c.Format())
	}
	if nativeapi.DeterministicCompilationVerifierEnabled {
		nativeapi.VerifyOrSetDeterministicCompilationContextValue(ctx, "After Finalization", c.Format())
	}
