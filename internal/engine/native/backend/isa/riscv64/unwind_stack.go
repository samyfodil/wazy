...
// The frame walk is identical to arm64's, because the frame layout is: both
// backends chase a frame_size word at SP rather than a frame-pointer chain, so
// there is nothing architecture-specific left in the traversal. Duplicated
// rather than shared to keep each backend's frame contract stated where its
// prologue lives; if a third backend adopts the same layout it is worth
// extracting.
