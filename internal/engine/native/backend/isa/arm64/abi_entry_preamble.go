...
func (m *machine) constructEntryPreamble(sig *ssa.Signature) (root *instruction) {
	...
	abi.Init(sig, intParamResultRegs, floatParamResultRegs, regalloc.RegTypeFloat)
	...
	// Next, save the current FP, SP and LR into the native.executionContext:
	...
	cur = m.loadOrStoreAtExecutionContext(fpVReg, nativeapi.ExecutionContextOffsetOriginalFramePointer, true, cur)
	cur = m.move64(tmpRegVReg, spVReg, cur)
	cur = m.loadOrStoreAtExecutionContext(tmpRegVReg, nativeapi.ExecutionContextOffsetOriginalStackPointer, true, cur)
	cur = m.loadOrStoreAtExecutionContext(lrVReg, nativeapi.ExecutionContextOffsetGoReturnAddress, true, cur)
	...
	cur = m.loadOrStoreAtExecutionContext(fpVReg, nativeapi.ExecutionContextOffsetOriginalFramePointer, false, cur)
	cur = m.loadOrStoreAtExecutionContext(tmpRegVReg, nativeapi.ExecutionContextOffsetOriginalStackPointer, false, cur)
	cur = m.move64(spVReg, tmpRegVReg, cur)
	cur = m.loadOrStoreAtExecutionContext(lrVReg, nativeapi.ExecutionContextOffsetGoReturnAddress, false, cur)
...
func (m *machine) loadOrStoreAtExecutionContext(d regalloc.VReg, offset nativeapi.Offset, store bool, prev *instruction) *instruction {
	instr := m.allocateInstr()
	mode := m.amodePool.Allocate()
