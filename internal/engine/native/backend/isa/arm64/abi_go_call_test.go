...
func TestMachine_CompileGoFunctionTrampoline(t *testing.T) {
	for _, tc := range []struct {
		...
		{
			name:                 "listener",
			exitCode:             nativeapi.ExitCodeCallListenerBefore,
...
`,
		...
		{
			name:     "go call",
			exitCode: nativeapi.ExitCodeCallGoFunctionWithIndex(100, false),
			sig: &ssa.Signature{
...
`,
		...
		{
			name:     "grow memory",
			exitCode: nativeapi.ExitCodeGrowMemory,
			sig: &ssa.Signature{
...
`,
		},
		{
			...
			exitCode: nativeapi.ExitCodeTryTableEnter,
			sig: &ssa.Signature{
