...
func (m *machine) resolveRelativeAddresses(ctx context.Context) {
	...
	for {
		...
		for i, pos := range m.orderedSSABlockLabelPos {
			...
			if nativeapi.PerfMapEnabled {
				if size > 0 {
					nativeapi.PerfMap.AddModuleEntry(fnIndex, offset, uint64(size), fmt.Sprintf("%s:::::%s", fn, labelPosToLabel[pos]))
				}
			}
			offset += size
		}
...
// check can be elided. A leaf (makes no call of any kind -- so
// maxRequiredStackSizeForCalls stays 0, see prepareCall) whose frame fits
// within the H7 MARGIN its caller already reserved below it cannot overflow:
// by the invariant in backend/go_call.go (point 3) every callee's frame base
// sits >= MARGIN above stackBottomPtr, and a leaf never re-establishes that
// margin for a callee (it has none), so eliding its check cannot compromise a
// deeper frame.
