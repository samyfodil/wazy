...
// between the two forms. Sharing one exit sequence per trap kind costs the
// per-site trap address, which is what a DWARF stack trace maps back to source,
// so a site that carries a source offset must keep its sequence inline. Without
// the guard every trap in a debug build reports the island's address instead --
// a silent loss no golden covers, since the goldens are built without offsets.
