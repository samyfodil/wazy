...
func encodeExitSequence(c backend.Compiler, ctxReg regalloc.VReg) {
	...
	restoreLr := encodeLoadOrStore(
		...
		addressMode{
			...
			rn:   ctxReg,
			imm:  nativeapi.ExecutionContextOffsetGoReturnAddress.I64(),
		},
	)
	...
	restoreFp := encodeLoadOrStore(
		...
		addressMode{
			kind: addressModeKindRegUnsignedImm12,
			rn:   ctxReg,
			imm:  nativeapi.ExecutionContextOffsetOriginalFramePointer.I64(),
		},
	)
	...
	restoreSpToTmp := encodeLoadOrStore(
		...
		addressMode{
			kind: addressModeKindRegUnsignedImm12,
			rn:   ctxReg,
			imm:  nativeapi.ExecutionContextOffsetOriginalStackPointer.I64(),
		},
	)
