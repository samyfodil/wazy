...
func (m *machine) lowerIRem(execCtxVReg regalloc.VReg, rd, rn regalloc.VReg, rm operand, _64bit, signed bool) {
	...
	// Check if rm is zero:
	m.exitIfNot(execCtxVReg, registerAsRegNotZeroCond(rm.nr()), _64bit, nativeapi.ExitCodeIntegerDivisionByZero)
...
func (m *machine) lowerIDiv(execCtxVReg, rd regalloc.VReg, rn, rm operand, _64bit, signed bool) {
	...
	if signed {
		...
		m.exitIfNot(execCtxVReg, vs.invert().asCond(), false, nativeapi.ExitCodeIntegerOverflow)
	}
}
...
func (m *machine) exitIfNot(execCtxVReg regalloc.VReg, c cond, cond64bit bool, code nativeapi.ExitCode) {
...
func (m *machine) lowerFpuToInt(rd regalloc.VReg, rn operand, ctx regalloc.VReg, signed, src64bit, dst64bit, nonTrapping bool) {
	...
	if !nonTrapping {
		...
		m.exitIfNot(execCtx, vc.asCond(), false, nativeapi.ExitCodeInvalidConversionToInteger)
...
func (m *machine) lowerExitWithCode(execCtxVReg regalloc.VReg, code nativeapi.ExitCode) {
	...
	*mode = addressMode{
		...
		rn:   execCtxVReg, imm: nativeapi.ExecutionContextOffsetExitCodeOffset.I64(),
	}
	...
	*mode2 = addressMode{
		kind: addressModeKindRegUnsignedImm12,
		rn:   execCtxVReg, imm: nativeapi.ExecutionContextOffsetStackPointerBeforeGoCall.I64(),
	}
	...
	*mode3 = addressMode{
		kind: addressModeKindRegUnsignedImm12,
		rn:   execCtxVReg, imm: nativeapi.ExecutionContextOffsetGoCallReturnAddress.I64(),
	}
