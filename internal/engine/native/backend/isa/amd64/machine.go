...
func NewBackend() backend.Machine {
	m := &machine{
		...
		amodePool:                           nativeapi.NewPool[amode](nil),
...
type (
	...
	machine struct {
		...
		amodePool nativeapi.Pool[amode]
...
func (m *machine) allocateExitInstructions(execCtx, exitCodeReg regalloc.VReg) (saveRsp, saveRbp, setExitCode *instruction) {
	saveRsp = m.allocateInstr().asMovRM(
		rspVReg,
		newOperandMem(m.newAmodeImmReg(nativeapi.ExecutionContextOffsetStackPointerBeforeGoCall.U32(), execCtx)),
		8,
	)

	saveRbp = m.allocateInstr().asMovRM(
		rbpVReg,
		newOperandMem(m.newAmodeImmReg(nativeapi.ExecutionContextOffsetFramePointerBeforeGoCall.U32(), execCtx)),
		8,
	)
	setExitCode = m.allocateInstr().asMovRM(
		exitCodeReg,
		newOperandMem(m.newAmodeImmReg(nativeapi.ExecutionContextOffsetExitCodeOffset.U32(), execCtx)),
		4,
	)
	return
}

func (m *machine) lowerExitWithCode(execCtx regalloc.VReg, code nativeapi.ExitCode) (afterLabel label) {
	exitCodeReg := rbpVReg
	...
	saveRip := m.allocateInstr().asMovRM(
		ripReg,
		newOperandMem(m.newAmodeImmReg(nativeapi.ExecutionContextOffsetGoCallReturnAddress.U32(), execCtx)),
		8,
	)
	m.insert(saveRip)
...
func (m *machine) requiredStackSize() int64 {
	...
	// small (imm8 for typical frames) instead of crossing the amd64
	// imm8->imm32 encoding boundary. See the full invariant proof on
...
func (m *machine) lowerIDivRemSequenceAfterRegAlloc(i *instruction) {
	...
	nz := m.lowerExitWithCode(execCtx, nativeapi.ExitCodeIntegerDivisionByZero)
	...
	if signed {
		...
		if isDiv {
			...
			end := m.lowerExitWithCode(execCtx, nativeapi.ExitCodeIntegerOverflow)
...
func (m *machine) lowerFcvtToSintSequenceAfterRegalloc(i *instruction) {
	...
	if sat {
		...
		// If non-sat, NaN, trap.
		m.lowerExitWithCode(execCtx, nativeapi.ExitCodeInvalidConversionToInteger)
		...
		m.lowerExitWithCode(execCtx, nativeapi.ExitCodeIntegerOverflow)
