...
// L0, so the last one encoded overwrote the entry block's resolved offset.
...
func TestMachine_unlabeledNopDoesNotClaimLabelZero(t *testing.T) {
	...
	l, ok := m.allocateInstr().asNop0WithLabel(0).nop0Label()
	require.True(t, ok)
	require.Equal(t, label(0), l)
}

// TestMachine_entryBlockOffsetSurvivesEncode is the observable consequence:
// after encoding a function whose later blocks contain real instructions, the
// entry block must still start at offset 0.
func TestMachine_entryBlockOffsetSurvivesEncode(t *testing.T) {
	...
	for i := 0; i < 4; i++ {
		mov := m.allocateInstr()
		mov.asMovRR(raxVReg, rcxVReg, true)
		m.insert(mov)
	}
