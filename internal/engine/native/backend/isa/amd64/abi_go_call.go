...
func (m *machine) CompileGoFunctionTrampoline(exitCode nativeapi.ExitCode, sig *ssa.Signature, needModuleContextPtr bool) []byte {
	...
	if needModuleContextPtr {
		...
		mem := m.newAmodeImmReg(
			nativeapi.ExecutionContextOffsetGoFunctionCallCalleeModuleContextOpaque.U32(),
			execCtrPtr)
...
func (m *machine) storeReturnAddressAndExit(cur *instruction, execCtx regalloc.VReg) *instruction {
	...
	readRip.asLEA(newOperandLabel(l), ripReg, true)
	return cur
}
...
func (m *machine) CompileStackGrowCallSequence() []byte {
	...
	cur = linkInstr(cur, m.allocateInstr().asImm(exitCodeReg, uint64(nativeapi.ExitCodeGrowStack), false))
...
// callee-saved registers from native.executionContext.savedRegisters
...
func (m *machine) insertStackBoundsCheck(requiredStackSize int64, cur *instruction) *instruction {
	...
	cur = linkInstr(cur, m.allocateInstr().asCmpRmiR(true,
		newOperandMem(m.newAmodeImmReg(nativeapi.ExecutionContextOffsetStackBottomPtr.U32(), raxVReg)),
		rspVReg, true))
	...
	cur = linkInstr(cur, m.allocateInstr().asMovRM(r15VReg,
		newOperandMem(m.newAmodeImmReg(nativeapi.ExecutionContextOffsetStackGrowRequiredSize.U32(), raxVReg)), 8))
	...
	cur = linkInstr(cur, m.allocateInstr().asCallIndirect(newOperandMem(m.newAmodeImmReg(
		nativeapi.ExecutionContextOffsetStackGrowCallTrampolineAddress.U32(), raxVReg)), nil))
