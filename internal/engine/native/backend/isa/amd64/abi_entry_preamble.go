...
func (m *machine) saveOriginalRSPRBP(cur *instruction) *instruction {
	// 		mov %rbp, nativeapi.ExecutionContextOffsetOriginalFramePointer(%executionContextPtrReg)
	// 		mov %rsp, nativeapi.ExecutionContextOffsetOriginalStackPointer(%executionContextPtrReg)
	cur = m.loadOrStore64AtExecutionCtx(executionContextPtrReg, nativeapi.ExecutionContextOffsetOriginalFramePointer, rbpVReg, true, cur)
	cur = m.loadOrStore64AtExecutionCtx(executionContextPtrReg, nativeapi.ExecutionContextOffsetOriginalStackPointer, rspVReg, true, cur)
	return cur
}
...
func (m *machine) restoreOriginalRSPRBP(cur *instruction) *instruction {
	// 		mov nativeapi.ExecutionContextOffsetOriginalFramePointer(%executionContextPtrReg), %rbp
	// 		mov nativeapi.ExecutionContextOffsetOriginalStackPointer(%executionContextPtrReg), %rsp
	cur = m.loadOrStore64AtExecutionCtx(savedExecutionContextPtr, nativeapi.ExecutionContextOffsetOriginalFramePointer, rbpVReg, false, cur)
	cur = m.loadOrStore64AtExecutionCtx(savedExecutionContextPtr, nativeapi.ExecutionContextOffsetOriginalStackPointer, rspVReg, false, cur)
	return cur
}
...
func (m *machine) loadOrStore64AtExecutionCtx(execCtx regalloc.VReg, offset nativeapi.Offset, r regalloc.VReg, store bool, prev *instruction) *instruction {
