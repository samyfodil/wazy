...
type (
	...
	compiledModule struct {
		*executables
		...
		// frame's SP from its FP (see nativeapi.ThrowFrame).
...
func (e *engine) CompileModule(ctx context.Context, module *wasm.Module, listeners []experimental.FunctionListener, ensureTermination bool) (err error) {
	...
	if nativeapi.DeterministicCompilationVerifierEnabled {
		ctx = nativeapi.NewDeterministicCompilationVerifierContext(ctx, len(module.CodeSection))
	}
	...
	if nativeapi.DeterministicCompilationVerifierEnabled {
		for i := 0; i < nativeapi.DeterministicCompilationVerifyingIter; i++ {
...
func (exec *executables) compileEntryPreambles(m *wasm.Module, machine backend.Machine, be backend.Compiler) {
	...
	for i, size := range sizes {
		...
		if nativeapi.PerfMapEnabled {
			typ := &m.TypeSection[i]
			nativeapi.PerfMap.AddEntry(uintptr(unsafe.Pointer(ptr)),
...
func (e *engine) compileModule(ctx context.Context, module *wasm.Module, listeners []experimental.FunctionListener, ensureTermination bool) (*compiledModule, error) {
	...
	if nativeapi.PerfMapEnabled {
		nativeapi.PerfMap.Flush(uintptr(unsafe.Pointer(&executable[0])), cm.functionOffsets)
	}
...
func functionContext(ctx context.Context, module *wasm.Module, fnum int, fidx wasm.Index) context.Context {
	if nativeapi.NeedFunctionNameInContext {
		def := module.FunctionDefinition(fidx)
		name := def.DebugName()
		if len(def.ExportNames()) > 0 {
			name = def.ExportNames()[0]
		}
		ctx = nativeapi.SetCurrentFunctionName(ctx, fnum, fmt.Sprintf("[%d/%d]%s", fnum, len(module.CodeSection)-1, name))
	}
	return ctx
}
...
func (r *engineRelocator) appendFunction(
	...
	if len(ehEntries) > 0 {
		rebased := make([]nativeapi.EhEntry, len(ehEntries))
		for i, e := range ehEntries {
			clauses := make([]nativeapi.EhClause, len(e.Clauses))
			for j, c := range e.Clauses {
				c.LandingPad += uintptr(r.totalSize)
				clauses[j] = c
			}
			rebased[i] = nativeapi.EhEntry{
	...
	if nativeapi.PrintMachineCodeHexPerFunction {
		fmt.Printf("[[[machine code for %s]]]\n%s\n\n", nativeapi.GetCurrentFunctionName(ctx), hex.EncodeToString(body))
	}
...
func (e *engine) compileLocalWasmFunction(
	...
	be backend.Compiler,
	needListener bool,
) (body []byte, rels []backend.RelocationInfo, ehEntries []nativeapi.EhEntry, frameSize int64, err error) {
	...
	// Lower Wasm to SSA.
	fe.LowerToSSA()
	if nativeapi.PrintSSA && nativeapi.PrintEnabledIndex(ctx) {
		fmt.Printf("[[[SSA for %s]]]%s\n", nativeapi.GetCurrentFunctionName(ctx), ssaBuilder.Format())
	}

	if nativeapi.DeterministicCompilationVerifierEnabled {
		nativeapi.VerifyOrSetDeterministicCompilationContextValue(ctx, "SSA", ssaBuilder.Format())
	}

	// Run SSA-level optimization passes.
	ssaBuilder.RunPasses()

	if nativeapi.PrintOptimizedSSA && nativeapi.PrintEnabledIndex(ctx) {
		fmt.Printf("[[[Optimized SSA for %s]]]%s\n", nativeapi.GetCurrentFunctionName(ctx), ssaBuilder.Format())
	}

	if nativeapi.DeterministicCompilationVerifierEnabled {
		nativeapi.VerifyOrSetDeterministicCompilationContextValue(ctx, "Optimized SSA", ssaBuilder.Format())
	}
...
// into nativeapi.EhEntry values with function-body-relative offsets, using
...
func buildEhEntries(pending []frontend.EhPendingEntry, blockOffsets []backend.CompiledBlockOffset, bodyLen int64) []nativeapi.EhEntry {
	if len(pending) == 0 {
		return nil
	}
	...
	var out []nativeapi.EhEntry
	for _, p := range pending {
		clauses := make([]nativeapi.EhClause, len(p.Clauses))
		...
		for i, pc := range p.Clauses {
			...
			clauses[i] = nativeapi.EhClause{Kind: pc.Kind, TagIndex: pc.TagIndex, LandingPad: uintptr(off)}
		}
		if unreachableEntry {
			continue
		}
		...
		flush := func(end int64) {
			if inRun {
				out = append(out, nativeapi.EhEntry{
...
func (e *engine) compileHostModule(ctx context.Context, module *wasm.Module, listeners []experimental.FunctionListener) (*compiledModule, error) {
	...
	for i := range module.CodeSection {
		...
		// We can relax until the index fits together in ExitCode as we do in nativeapi.ExitCodeCallGoModuleFunctionWithIndex.
		...
		var exitCode nativeapi.ExitCode
		fn := c.GoFunc
		switch fn.(type) {
		case api.GoModuleFunction:
			exitCode = nativeapi.ExitCodeCallGoModuleFunctionWithIndex(i, withListener)
		case api.GoFunction:
			exitCode = nativeapi.ExitCodeCallGoFunctionWithIndex(i, withListener)
		}
		...
		if nativeapi.PerfMapEnabled {
			...
			nativeapi.PerfMap.AddModuleEntry(i,
				int64(totalSize),
				uint64(len(body)),
...
func (e *engine) compileSharedFunctions() {
	...
	if nativeapi.PerfMapEnabled {
		...
		nativeapi.PerfMap.AddEntry(uintptr(unsafe.Pointer(fns.tableGrowAddress)), uint64(sizes[1]), "table_grow_trampoline")
		nativeapi.PerfMap.AddEntry(uintptr(unsafe.Pointer(fns.checkModuleExitCodeAddress)), uint64(sizes[2]), "check_module_exit_code_trampoline")
		nativeapi.PerfMap.AddEntry(uintptr(unsafe.Pointer(fns.refFuncAddress)), uint64(sizes[3]), "ref_func_trampoline")
		nativeapi.PerfMap.AddEntry(uintptr(unsafe.Pointer(fns.stackGrowAddress)), uint64(sizes[4]), "stack_grow_trampoline")
		nativeapi.PerfMap.AddEntry(uintptr(unsafe.Pointer(fns.memoryWait32Address)), uint64(sizes[5]), "memory_wait32_trampoline")
		nativeapi.PerfMap.AddEntry(uintptr(unsafe.Pointer(fns.memoryWait64Address)), uint64(sizes[6]), "memory_wait64_trampoline")
		nativeapi.PerfMap.AddEntry(uintptr(unsafe.Pointer(fns.memoryNotifyAddress)), uint64(sizes[7]), "memory_notify_trampoline")
		nativeapi.PerfMap.AddEntry(uintptr(unsafe.Pointer(fns.throwAllocTrampolineAddress)), uint64(sizes[8]), "throw_alloc_trampoline")
		nativeapi.PerfMap.AddEntry(uintptr(unsafe.Pointer(fns.throwTrampolineAddress)), uint64(sizes[9]), "throw_trampoline")
