...
func TestStatFile(t *testing.T) {
	...
	tmpDirF := requireOpenFile(t, tmpDir, sys.O_RDONLY, 0)
	defer tmpDirF.Close()
	...
	file := path.Join(tmpDir, "file")
	require.NoError(t, os.WriteFile(file, nil, 0o400))
	fileF := requireOpenFile(t, file, sys.O_RDONLY, 0)
	defer fileF.Close()
	...
	subdir := path.Join(tmpDir, "sub")
	require.NoError(t, os.Mkdir(subdir, 0o500))
	subdirF := requireOpenFile(t, subdir, sys.O_RDONLY, 0)
	defer subdirF.Close()
...
func Test_StatFile_times(t *testing.T) {
	...
	for _, tt := range tests {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			f := requireOpenFile(t, file, sys.O_RDONLY, 0)
			defer f.Close()
...
func TestStatFile_dev_inode(t *testing.T) {
	tmpDir := t.TempDir()
	d := requireOpenFile(t, tmpDir, sys.O_RDONLY, 0)
	defer d.Close()

	path1 := path.Join(tmpDir, "1")
	f1 := requireOpenFile(t, path1, sys.O_CREAT, 0o666)
	defer f1.Close()

	path2 := path.Join(tmpDir, "2")
	f2 := requireOpenFile(t, path2, sys.O_CREAT, 0o666)
	defer f2.Close()

	pathLink2 := path.Join(tmpDir, "link2")
	err := os.Symlink(path2, pathLink2)
	require.NoError(t, err)
	l2 := requireOpenFile(t, pathLink2, sys.O_RDONLY, 0)
	defer l2.Close()
	...
	f1 = requireOpenFile(t, path2, sys.O_RDONLY, 0)
	defer f1.Close()
...
func requireNotDir(t *testing.T, d sys.File, st sys.Stat_t) {
...
func requireDir(t *testing.T, d sys.File, st sys.Stat_t) {
	// Verify cached state is correct
	isDir, errno := d.IsDir()
	require.EqualErrno(t, 0, errno)
...
func requireDevIno(t *testing.T, f sys.File, st sys.Stat_t) {
