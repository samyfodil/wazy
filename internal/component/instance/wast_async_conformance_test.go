...
var asyncWastSuites = []asyncSuite{
	...
	{name: "trap-on-reenter", skipReason: "deferred: BOTH originally-documented binder gaps are now fixed and verified (Instantiate succeeds for all 3 $Parent variants, confirmed by re-running: no more 'component instance index 0 out of range of 0 imported instances' or 'func arg index 0 is not an instance-export alias'). (a) computeCanonHostFunc's ComponentFuncFromAlias branch (graph.go, canon LOWER case) now branches al.InstanceIdx vs numImported exactly like outerFuncArgImport already did for instantiate-args: >= numImported resolves the SIBLING nested component instance's export STATICALLY (resolveStaticExportFuncDesc, composition.go -- fd/resolve need no instantiation, only comp.ResolveType/typeResolver against the nested component's own static definition) since the sibling may not be instantiated yet at this point in the core-instance loop (instantiateNestedInstances runs after it) -- the actual live *Instance/*boundExport is filled in lazily (cfg.pendingDelegates, delegatingHostImportDeferred) once instantiateNestedInstances returns, always before any guest call could reach it. (b) outerFuncArgImport (composition.go) now also accepts a func-sort instantiate-arg that is the outer component's own directly-declared LOCAL func (a plain canon lift, not an alias at all) by building it via bindFuncExportGraph exactly like a real export would be, then delegating to it via delegatingHostImport(in, ...) -- the outer's own core func is always ready by the time instantiateNestedInstances runs. Both fixes are additive/dead-before-now (the previously-unconditional importInterfaceName call and the previously-unconditional alias-only check already hard-failed these exact shapes, so no currently-passing suite's behavior changes) -- verified via the full oracle/resources/wast/whole-repo/-race gates alongside this change, all still green. BUT the suite still doesn't pass: all 3 calls now reach a REAL trap attempt instead of an Instantiate error, and it is NOT the poisoning mechanism (builtin-trap-poisons-instance) that's missing -- it's a separate, deeper gap in the re-entrancy guard itself. Traced against the reference (testdata/definitions.py's Store.lift/Task.enter_from/leave_to, Store.tick): mayEnter is meant to stay false for a callback-based async task's ENTIRE lifetime (start to resolve), including while the task is PARKED between YIELD/WAIT dispatches (the reference's fiber-based canon_lift keeps its Python call frame suspended-but-not-returned the whole time) -- but wazy's mayEnter (task.go's enterRun/leaveRun) is DELIBERATELY transient, bracketing only each individual CallWithStack (documented at task.go:272-277, Phase 3 §1.2: 'unlike Phase 1/2's ... mayEnter false only between an enterRun and its matching leaveRun'), because wazy's callback-loop task model has no true fiber/stack-switch to hold a frame open across a YIELD -- so between $Parent's own 'c' task yielding and $Child's callback resuming and calling back into $Parent's $a, mayEnter has already flipped back to true and the call goes straight into $a's real core code (which then unreachables for real, a DIFFERENT trap than the spec's re-entrancy trap). Fixing this needs new instance-level state tracking 'has an outstanding not-yet-resolved task' independent of the transient CallWithStack bracket, PLUS the reference's entering_set/self_and_ancestors exclusion (a nested child calling back into its own static parent is allowed in a plain synchronous nested-call chain; only an async, scheduler-tick-mediated re-entry after a yield should trap) -- getting that ancestor-exclusion subtlety wrong risks over-trapping and breaking currently-passing composition suites (async-calls-sync, cancel-subtask, partial-stream-copies, deadlock, ...) that legitimately chain calls between nested instances. A genuine change to the shared task/re-entrancy model, touching every async suite's call path -- too invasive to attempt safely in this session; left skipped for this new, precise, narrower reason"},
