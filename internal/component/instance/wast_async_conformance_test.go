...
var asyncWastSuites = []asyncSuite{
	...
	{name: "sync-streams", skipReason: "UPSTREAM FIXTURE BUG (confirmed against WebAssembly/component-model main, test/async/sync-streams.wast, 2026-07-18). Our whole runtime is correct here: $C.get/$C.set are PROMOTED guestTasks (callback-task parking, docs/component-model-async-final3-fable.md) whose mid-call gt.block parks instead of nested-driving; EVERY event-code/ordering assertion passes (both DROPPED|4<<4=0x41 overwrite-at-delivery races, both COMPLETED=0x40 non-blocking rendezvous, $C.get/set reach EXIT, $D.run reaches i32.const 42) -- verified by patching ONLY the one failing constant in a SCRATCH .wasm copy (never the vendored fixture) and seeing the whole suite pass. The lone failure is $C.set's `i32.load($bufp) == 0x89abcdef` (wast:62), and the fixture is simply WRONG: the two halves are meant to be symmetric -- in the first half $C.get STORES 0x01234567 at its offset 16 (wast:36-37) then stream.writes it, and $D reads+checks 0x01234567 (wast:128); in the second half $D.run is SUPPOSED to store 0x89abcdef at its offset 16 before its stream.write (wast:141-142) so $C.set reads+checks it (wast:62) -- but $D.run is MISSING the `(i32.store (local.get $bufp) (i32.const 0x89abcdef))` line that mirrors $C.get's wast:37. $D therefore stream.writes its zero-initialized offset 16, $C.set reads 0x00000000, and the assert traps. No i32.store of 0x89abcdef exists anywhere in either core module (wasm-tools print/dump), and upstream main is byte-identical (WebFetch of the raw .wast: $D.run has no such store). Cross-checked against the reference (definitions.py SharedStreamImpl.write / stream_copy): the reference run against this EXACT bytecode copies the identical zero bytes and fails too. NO correct implementation can pass this fixture as vendored -- it needs an upstream fix (add the missing store to $D.run). Do NOT patch the vendored fixture to force a pass (that would falsify the conformance claim). Skipped honestly as an upstream defect; report it upstream."},
