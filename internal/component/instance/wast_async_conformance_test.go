...
var asyncWastSuites = []asyncSuite{
	...
	{name: "big-interleaving-test", skipReason: "deferred: NOT the busy-state/concurrent-op traps this session added (drop-stream's 'cannot drop/remove busy stream', trap-if-done's DONE-state family, and the new 'cannot have concurrent operations active on a future/stream' COPYING-state text are all wired and, per this suite's own assert_trap lines 837/907/1408/1439/1595, exactly the ones it would exercise) -- Instantiate now succeeds after this session's graph.go fix (bindInstanceExportGraph didn't handle Instance.Kind==0x01, 'inline exports': this suite's `(instance $types (export \"event-kind\" (type $driver \"event-kind\")) ...)` re-exports $Driver's types with no nested-component instantiation at all -- fixed by binding only the func-sort members, matching the existing type-sort-export skip convention, since $types is never itself invoked). But `run` itself now fails EARLIER than any trap, on every single assert_return/assert_trap: 'type index 11 not found' (instance.go's resolveTypeRef). Root cause (confirmed via a decode-only probe): $Tester's OWN TypeSpace has only 6 entries (aliases of $Driver's event-kind/io-args/poll-expect/sub-args/sub-expect/command types, the same 6 pulled in by the $types instance above), but $run's OWN FuncDesc (realized via `(alias export $driver \"run\" (func $run))`) has a param type (`list<command>`) whose TypeRef indices go up to 11 -- these are NOT further alias entries in $Tester's TypeSpace, they are $Driver's OWN internal type indices (nested[2].Types has 35 entries, comfortably covering 11), used AS-IS because an alias doesn't recompute a func's internal type references, it only gives the SAME entity a new external name/index. bindFuncExportGraph passes a single `resolve abi.Resolver` (built from $Tester's own comp.ResolveType) uniformly to every func it binds, whether the func is $Tester's own or realized via alias -- correct for every OTHER currently-passing suite because none of them alias a func whose param/result type is non-primitive (resolveTypeRef's fast path returns immediately for a primitive TypeRef, never calling resolve() at all, so the bug was never reachable before). Fixing this needs bindFuncExportGraph (or componentFunc's aliasTarget machinery) to resolve an ALIASED func's param/result TypeRefs through its DEFINING component's own resolver, not the aliasing component's -- a real, separate binder gap, out of scope for this session's trap work"},
	...
	{name: "drop-cross-task-borrow", skipReason: "deferred: NOT the borrow-leak trap itself -- task.go's returnValues already has trap_if(num_borrows > 0) (\"task.return: %d borrowed handle(s) still lent to subtasks\", reachable and correctly wired for a callback-based async export) and the message just needed updating to contain the spec's \"borrow handles still remain at the end of the call\" (done this session). The suite still fails EARLIER, before that trap is ever reached: a real resource-type-tag mismatch bug in the composition/graph resource canonicalizer (graph.go's resCanon/ResourceDefIndex chain, instance.go's resolveHandleArg) for this specific 3-component sibling shape -- $E imports type R BOTH directly from $C ((alias export $c \"R\" (type $R))) AND indirectly through $D's re-export of the same underlying $C.R ((import \"d\" (instance (export \"R\" (type $R (sub resource)))))); a handle $E mints via $c's R-new and passes as a borrow arg to $D's async export traps with \"handle 1 belongs to resource type 1, not 2\" (resource.go's Rep/TakeOwn tag check) -- the two alias paths to the same underlying resource definition are not being canonicalized to the same tag. Confirmed by re-running after this session's message-text fix: identical bind-succeeds-but-call-traps failure, unrelated to and upstream of the borrow-leak trap this suite is nominally testing. A real bug in the resource-identity reduction across multi-hop nested-component aliasing, not a fail-loud protocol trap -- out of this session's scope"},
	{name: "drop-stream"},
	{name: "drop-subtask"},
