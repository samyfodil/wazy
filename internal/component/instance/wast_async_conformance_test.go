...
var asyncWastSuites = []asyncSuite{
	...
	{name: "trap-if-done"},
	{name: "trap-if-sync-and-waitable-set", skipReason: "deferred: decode fails, 'async canon kind 0x27 not yet supported' -- this suite's $Main uses thread.new-indirect/thread.yield-then-resume (the real multi-fiber thread API) to run its sync waitable-set-membership probes concurrently with the async blocking call under test; same pre-existing decoder gap as cancellable/sync-barges-in/trap-if-block-and-sync ('async canon kind 0xc/0x27 not yet supported'), not a gap in the sync-and-waitable-set trap logic itself (verified: every trap_if this suite would exercise -- streamCopyHostFunc/futureCopyHostFunc/cancelCopyHostFunc's e.inWaitableSet() && !async guard -- already fires correctly for a directly-reachable synchronous copy/cancel on a waitable-set member, see trap-if-transfer-in-waitable-set's passing sibling checks; this suite just can't get past Instantiate to reach them)"},
