...
var asyncWastSuites = []asyncSuite{
	...
	{name: "passing-resources", skipReason: "deferred: NOT a fail-loud-trap gap -- Instantiate itself fails before any trap logic is reachable, at resolveStreamOrFutureElem's bind-time ceiling (stream_builtins.go's elemContainsResourceHandle): this suite's stream element type is `own $R`, a resource handle, and wazy's Phase 2 stream/future implementation deliberately rejects any stream/future element type that carries an own/borrow ('stream/future element type contains an own/borrow resource handle, which is not supported by this milestone') because copyElements (stream.go) has no per-element handle-table transfer -- the numeric-fast-path/general-lift-lower split it does have only moves plain values, never re-homes a resource handle from the writer's table into the reader's. This suite needs that transfer machinery built (mint a fresh handle in the reader's table for each `own` element copied, release it from the writer's), not a trap; genuinely deferred, confirmed by re-running after this session's trap work (still fails at the identical bind-time error)"},
