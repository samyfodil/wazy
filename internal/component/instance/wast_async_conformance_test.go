...
var asyncWastSuites = []asyncSuite{
	...
	{name: "big-interleaving-test", skipReason: "deferred: the binder bug this suite was ORIGINALLY skipped for (bindFuncExportGraph's isLift==false alias-reuse path returning a boundExport whose fd carries the DEFINING component's own TypeRef indices) is FIXED -- confirmed root cause was actually in the TEST HARNESS, not production dispatch: boundExport.paramSteps/resultType/paramTypes (finalizeBoundExport, used by every real lower/lift call) were already correctly baked against the DEFINING component's ($Driver's) own resolver at $Driver's own bind time and never re-resolved a TypeRef at call time, so 'run's actual param/result ABI was always correct; only wast_conformance_test.go's wastConvert (the JSON-fixture-to-abi.Value test helper invokeWast/expectedWast use, walking fd's nested list/record/variant TypeRefs on demand to interpret a spec-test literal) used the CALLING Instance's ($Tester's, only 6 TypeSpace entries) resolver instead of the DEFINING component's, hitting 'type index 11 not found' the moment a suite aliased a func with a non-primitive param (list<command> here) -- the first suite to do so. Fixed by adding boundExport.resolve (set once in finalizeBoundExport, alongside paramTypes/resultType) and threading it through wastConvert instead of implicitly using in.resolve; verified against the whole $Tester TypeSpace/$Driver TypeSpace split (19 of 45 assert_returns now convert and run correctly that previously failed 100% at conversion, before ever reaching the actual call). But the suite still doesn't pass as a whole: 23 of 45 assert_returns now fail with 'called outside an active async task' (waitable-set.poll, backpressure.inc, context.get/set) or a propagated 'unreachable' from the same -- a SEPARATE, pre-existing, already-documented gap (see dont-block-start/empty-wait/deadlock's entries above): $Driver's \"run\" and $Testee's several exports (poll, poll-readable, bp-inc, ...) are all PLAIN SYNC canon lifts (no async option), so they dispatch via invoke() with no guestTask at all; when their core code calls an async builtin (waitable-set.poll, backpressure.inc, context.get/set -- all legal per spec even in a sync-labeled call, since the spec's Task exists for every call regardless of the async option) it hits requireActiveTask's generic trap because wazy only ever constructs a task for an asyncCallback/stackful lift, never an implicit one for a plain sync export. Fixing THAT (an implicit Task for every call, not just async-lifted ones) is a cross-cutting architecture change touching every sync call path in the runtime (risking every currently-passing suite, including the whole sync wast/resources harness), squarely the same class of deferred, bigger-design gap as the callback-task-parking items -- not attempted here"},
	{name: "builtin-trap-poisons-instance", skipReason: "deferred: instance-poisoning-after-trap ITSELF is now implemented and verified working (Instance.poisoned, instance.go/async_lift.go/guest_task.go/stackful_task.go) -- of this suite's 4 assert_traps, the 3 that actually exercise poisoning/the busy-stream trap all pass (line 10's post-unreachable 'cannot enter component instance' on the plain-sync $Unreachable.f export, and line 38/39's busy-stream trap + its own follow-up poisoning check on $BuiltinTrap.f). The ONLY remaining failure is line 9's FIRST assert_trap, on the very first (pre-poisoning) call to $Unreachable.f: it expects the verbatim wasmtime-style text 'wasm trap: wasm `unreachable` instruction executed', but wazy's core engine reports unreachable traps as 'wasm error: unreachable\\nwasm stack trace: ...' (internal/wasmruntime's ErrRuntimeUnreachable wording) -- a pre-existing, cosmetic trap-TEXT convention difference between wazy's engine and wasmtime's, unrelated to async/poisoning (confirmed: no other suite, sync or async, expects this exact wasmtime phrasing -- every other 'unreachable' assert_trap in the vendored suites uses the short substring 'unreachable', which wazy's own wording already contains). Fixing it would mean changing internal/wasmruntime's trap-message format globally (used by every wazy caller, not just component-model), which is out of scope for this session's async work and risks rippling far beyond it"},
	...
	{name: "cancel-subtask"},
	{name: "closed-stream"},
	{name: "cross-abi-calls", skipReason: "deferred: cross-instance realloc on canon lift (docs/component-model-async-final3-fable.md Feature 2) is now FIXED -- graph.go's resolveReallocFuncGraph no longer rejects a lift whose realloc option targets a different core instance than the lift's own core func (boundExport.reallocMod, finalizeBoundExport), confirmed: the suite's original bind-time error ('export \"sync-17-param\": realloc core func targets a different core instance...') is gone. Instantiate now fails on a SEPARATE, pre-existing, already-documented gap instead: $Top's `func-5-param` async-typed import has 5 flat params, one over async_host_import.go's maxFlatAsyncParams==4 ('async import \"func-5-param\" func \"\" has 5 flat param(s), exceeding the async flat limit (4); spilling params to memory is not supported by this milestone') -- async-LOWERED imports (unlike sync-lowered/lifted exports, which already spill via paramsSpill) have no memory-spill path for >4 flat params at all, a real unimplemented milestone ceiling unrelated to realloc. Every module_instance in this suite binds func-5-param eagerly at Instantiate, so this blocks the whole suite (0 assertions reachable), not just the 5-param-and-up assertions. Fixing it means building an async-import param-spill path (buildAsyncHostWrapper/buildHostParamPlans), a real feature, not a Feature-2-sized plumbing change -- out of scope here, left skipped for this narrower, precise reason"},
	{name: "cross-task-future"},
	{name: "deadlock"},
	{name: "dont-block-start"},
	{name: "drop-cross-task-borrow"},
	{name: "drop-stream"},
	{name: "drop-subtask"},
	{name: "drop-waitable-set"},
	{name: "empty-wait"},
	{name: "futures-must-write"},
	{name: "partial-stream-copies"},
	{name: "passing-resources"},
	{name: "same-component-stream-future"},
	{name: "sync-barges-in", skipReason: "deferred: same decode fix as cancellable (thread.yield, canon kind 0x0c, now decodes) -- Instantiate now fails at BIND time instead, 'core instance 1: inline export \"thread.yield\": references a canon of kind 0xc, which does not produce a core func'. Unlike cancellable, this suite's thread.yield call ($C's \"yielder\" export) IS reached from a STACKFUL (no-callback) lift, which does have real goroutine-based suspend/resume (stackful_task.go) -- so a thread.yield host func here is more plausible in principle than for a callback-lifted caller, but it is still a new async primitive (voluntary yield-and-resume mid-core-call, distinct from any existing blocking builtin) with no host func implementation at all; adding execution support (not just decoding the opcode) is out of this session's explicit scope (thread.* execution is not to be implemented) -- left skipped, decode gap fixed"},
	{name: "sync-streams", skipReason: "deferred: Feature 1 (callback-task parking, docs/component-model-async-final3-fable.md) IS implemented and IS the fix for the original deadlock -- exhaustively verified: $C.get/$C.set are now PROMOTED guestTasks (mayBlockSync, since $C binds sync stream.read/write) whose mid-call gt.block parks instead of nested-driving; every event code/ordering assertion in the fixture passes (both DROPPED|4<<4 = 0x41 event-overwrite-at-delivery races, both COMPLETED = 0x40 non-blocking rendezvous on $D's side, $C.get/set both reach EXIT, $D.run reaches its final i32.const 42) -- confirmed by patching ONLY the one failing assertion's constant in a scratch copy of the .wasm (never touching the vendored fixture) and observing the whole suite pass. The ONE remaining failure is $C.set's `i32.load($bufp) == 0x89abcdef` check: wasm-tools disassembly of the vendored sync-streams.0.wasm shows ZERO i32.store instructions and no data section anywhere targeting that address in either $C's or $D's core module (confirmed via `wasm-tools print`/`dump`), so $D's linear memory at the relevant offset is genuinely, correctly zero-initialized (spec-mandated) at the time of $D's second stream.write -- there is no legitimate data-flow path, in ANY implementation, that produces 0x89abcdef there. Cross-checked directly against the reference (internal/component/abi/testdata/definitions.py's SharedStreamImpl.write/WritableStreamEnd.drop, stream_copy's on_copy/on_copy_done wiring): byte-for-byte identical semantics to wazy's implementation (eager write-time copy, lazy/deferred reclaim, drop-overrides-undelivered-event) -- the reference run against this EXACT bytecode would copy the identical zero bytes. This is a vendored-fixture-level residual (the upstream WebAssembly/component-model test itself, byte-identical in a fresh upstream checkout), not a Feature 1 gap -- genuinely needs something beyond this session's three features (or an upstream fixture fix) to satisfy, so left honestly skipped rather than forced"},
	{name: "trap-if-block-and-sync", skipReason: "deferred: thread.index/thread.new-indirect/thread.suspend/thread.yield (canon kinds 0x26/0x27/0x29/0x0c) now all decode (binary/decoder.go, verified against this fixture's own bytes -- the old blanket 'async canon kind 0xc/0x27 not yet supported' is gone). Instantiate now fails at BIND time instead, every one of its 15 assert_returns hitting the identical 'core instance 2: inline export \"thread.yield\": references a canon of kind 0xc, which does not produce a core func' (graph.go's computeCanonHostFunc has no case for any of the 4). This is the suite the ORIGINAL skip reason already correctly called out as needing 'a genuine multi-fiber thread API' -- thread.new-indirect literally spawns a new thread of execution running an indirect-table func, which has no analog anywhere in wazy's task model (guestTask/stackfulTask both model exactly ONE thread of control per task); implementing it is a real, substantial runtime feature, deliberately out of scope (do not implement thread.* execution)"},
	{name: "trap-if-done"},
	{name: "trap-if-sync-and-waitable-set", skipReason: "deferred: thread.new-indirect/thread.yield-then-resume (canon kinds 0x27/0x2b) now decode (binary/decoder.go, verified against this fixture's own bytes). Instantiate now fails at BIND time instead of decode time, 'core instance 1: inline export \"thread.new-indirect\": references a canon of kind 0x27, which does not produce a core func' -- this suite's $Main genuinely spawns a concurrent thread of execution (thread.new-indirect) to run its sync waitable-set-membership probes WHILE the async blocking call under test is in flight, the same real multi-fiber thread API trap-if-block-and-sync needs; not implemented (do not implement thread.* execution). The sync-and-waitable-set trap logic itself was already separately verified correct (see trap-if-transfer-in-waitable-set's passing sibling checks) -- this suite still just can't get past Instantiate to reach it, now for a bind-time (not decode-time) reason"},
	{name: "trap-if-transfer-in-waitable-set"},
	{name: "trap-on-reenter", skipReason: "deferred: NOT an instance-poisoning gap (poisoning itself works -- see builtin-trap-poisons-instance) -- none of this suite's 3 $Parent variants ever reach a poisoning-relevant call at all; all 3 fail at Instantiate, for two SEPARATE, pre-existing composition/graph-engine gaps unrelated to async: (a) cases 1 and 2 (trap-on-reenter/trap-on-reenter.{0,1}.wasm): a plain top-level `(canon lower (func $child \"b_or_f\") ...)` in the OUTER component's OWN core-instance wiring, where $child is a locally-instantiated SIBLING nested component (not a host import) -- computeCanonHostFunc's ComponentFuncFromAlias branch (graph.go) unconditionally calls importInterfaceName (host-imported-instances only) instead of branching on al.InstanceIdx vs numImported the way outerFuncArgImport (composition.go) already correctly does for instantiate-ARGS, so it fails 'component instance index 0 out of range of 0 imported instances'. This is not a one-line branch fix, though: instantiateGraph builds the outer's OWN core instances (the loop that reaches this canon) BEFORE instantiateNestedInstances ever instantiates the sibling ($child) being referenced (confirmed via a decode-only probe of both .wasm files' ComponentFuncSpace/CoreInstances), so even routing correctly would dereference a sibling that doesn't exist yet -- fixing it needs the graph engine's core-instance vs nested-component-instance construction reordered by actual dependency, a structural change to instantiateGraph's phasing, not a local patch. (b) case 3 (trap-on-reenter.2.wasm): outerFuncArgImport only accepts a func-sort instantiate-arg that is itself an alias (to a host import or a sibling's export); this case's `(with \"f\" (func $f))` names $Parent's OWN directly-declared local func (a plain canon lift, not an alias at all), hitting 'func arg index 0 is not an instance-export alias'. Even fixing (b) alone wouldn't pass the suite (runAsyncWastSuite requires every command across all 3 modules to succeed), and (a) is the harder, out-of-scope one -- left skipped as a unit"},
	{name: "validate-no-async-abi-for-sync-type"},
