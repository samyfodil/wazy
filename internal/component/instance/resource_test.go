...
func TestLiftHostArgs_OwnHandle(t *testing.T) {
	...
	args, err := liftHostArgs(fd, resolve, []uint64{uint64(h)}, mod, tbl)
	if err != nil {
...
func TestLiftHostArgs_UnknownHandle(t *testing.T) {
	...
	tbl := newHandleTable()
	fd, resolve := synthFuncDesc([]binary.TypeDesc{binary.OwnDesc{ResourceType: 1}}, nil)
	if _, err := liftHostArgs(fd, resolve, []uint64{999}, mod, tbl); err == nil {
...
func TestLowerHostResults_OwnHandle(t *testing.T) {
	...
	stack := make([]uint64, 1)
	if err := lowerHostResults(ctx, fd, resolve, []abi.Value{uint32(55)}, stack, mod, tbl); err != nil {
...
func TestLowerHostResults_BorrowHandle(t *testing.T) {
	...
	stack := make([]uint64, 1)
	if err := lowerHostResults(ctx, fd, resolve, []abi.Value{uint32(66)}, stack, mod, tbl); err != nil {
		t.Fatalf("lowerHostResults: %v", err)
	}
	h := uint32(stack[0])
	rep, err := tbl.Rep(2, h)
