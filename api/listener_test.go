...
var _ api.FunctionListenerFactory = &recorder{}
...
func (r *recorder) Before(ctx context.Context, _ api.Module, def api.FunctionDefinition, _ []uint64, _ api.StackIterator) {
...
func (r *recorder) NewFunctionListener(definition api.FunctionDefinition) api.FunctionListener {
...
func TestFunctionListenerFactory(t *testing.T) {
	...
	ctx := api.WithFunctionListenerFactory(context.Background(), factory)
...
func TestMultiFunctionListenerFactory(t *testing.T) {
	...
	n := 0
	f := func(ctx context.Context, mod api.Module, def api.FunctionDefinition, params []uint64, stackIterator api.StackIterator) {
		n++
		i := 0
	...
	factory := api.MultiFunctionListenerFactory(
		api.FunctionListenerFactoryFunc(func(def api.FunctionDefinition) api.FunctionListener {
			return api.FunctionListenerFunc(f)
		}),
		api.FunctionListenerFactoryFunc(func(def api.FunctionDefinition) api.FunctionListener {
			return api.FunctionListenerFunc(f)
		}),
	...
	listener.Before(context.Background(), module, function, stack[2].Params, api.NewStackIterator(stack...))
...
func BenchmarkMultiFunctionListener(b *testing.B) {
	...
	tests := []struct {
		scenario string
		function func(context.Context, api.Module, api.FunctionDefinition, []uint64, api.StackIterator)
	}{
		{
			scenario: "simple function listener",
			function: func(ctx context.Context, mod api.Module, def api.FunctionDefinition, params []uint64, stackIterator api.StackIterator) {
			},
		},
		{
	...
	for _, test := range tests {
		b.Run(test.scenario, func(b *testing.B) {
			factory := api.MultiFunctionListenerFactory(
				api.FunctionListenerFactoryFunc(func(def api.FunctionDefinition) api.FunctionListener {
					return api.FunctionListenerFunc(test.function)
				}),
				api.FunctionListenerFactoryFunc(func(def api.FunctionDefinition) api.FunctionListener {
					return api.FunctionListenerFunc(test.function)
				}),
			...
			function := module.Function(0).Definition()
			listener := factory.NewFunctionListener(function)
			api.BenchmarkFunctionListener(b.N, module, stack, listener)
		})
	}
}
