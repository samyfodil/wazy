...
func Example_importResolver() {
	...
	for i := range mods {
		...
		ctx = api.WithImportResolver(ctx, func(name string) api.Module {
		...
		m.client, err = r.InstantiateModule(ctx, idcm, wazy.NewModuleConfig().WithName(fmt.Sprintf("m%d", i)))
		if err != nil {
			log.Panicln(err)
		}
