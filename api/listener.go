...
type FunctionListenerFactory interface {
	...
	NewFunctionListener(FunctionDefinition) FunctionListener
...
type FunctionListener interface {
	...
	Before(ctx context.Context, mod Module, def FunctionDefinition, params []uint64, stackIterator StackIterator)
	...
	After(ctx context.Context, mod Module, def FunctionDefinition, results []uint64)
	...
	//
	//   - api.Memory is meant for inspection, not modification.
	Abort(ctx context.Context, mod Module, def FunctionDefinition, err error)
}
...
type FunctionListenerFunc func(context.Context, Module, FunctionDefinition, []uint64, StackIterator)

// Before satisfies the FunctionListener interface, calls f.
func (f FunctionListenerFunc) Before(ctx context.Context, mod Module, def FunctionDefinition, params []uint64, stackIterator StackIterator) {
	f(ctx, mod, def, params, stackIterator)
}

// After is declared to satisfy the FunctionListener interface, but it does
// nothing.
func (f FunctionListenerFunc) After(context.Context, Module, FunctionDefinition, []uint64) {
}

// Abort is declared to satisfy the FunctionListener interface, but it does
// nothing.
func (f FunctionListenerFunc) Abort(context.Context, Module, FunctionDefinition, error) {
}
...
type FunctionListenerFactoryFunc func(FunctionDefinition) FunctionListener

// NewFunctionListener satisfies the FunctionListenerFactory interface, calls f.
func (f FunctionListenerFactoryFunc) NewFunctionListener(def FunctionDefinition) FunctionListener {
	return f(def)
}
...
func (multi multiFunctionListenerFactory) NewFunctionListener(def FunctionDefinition) FunctionListener {
...
func (multi *multiFunctionListener) Before(ctx context.Context, mod Module, def FunctionDefinition, params []uint64, si StackIterator) {
	multi.stack.base = si
...
func (multi *multiFunctionListener) After(ctx context.Context, mod Module, def FunctionDefinition, results []uint64) {
	for _, lstn := range multi.lstns {
		lstn.After(ctx, mod, def, results)
	}
}

func (multi *multiFunctionListener) Abort(ctx context.Context, mod Module, def FunctionDefinition, err error) {
	for _, lstn := range multi.lstns {
...
func (f internalFunction) Definition() FunctionDefinition {
	return f.definition
}
...
func NewStackIterator(stack ...StackFrame) StackIterator {
	si := &stackFrameIterator{
		...
		fndef: make([]FunctionDefinition, len(stack)),
	}
	for i := range stack {
...
func BenchmarkFunctionListener(n int, module Module, stack []StackFrame, listener FunctionListener) {
	if len(stack) == 0 {
