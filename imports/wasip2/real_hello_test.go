...
func TestRealHello_InstantiatesGraph(t *testing.T) {
	...
	if inst == nil {
		t.Fatal("Instantiate returned a nil *component.Instance with a nil error")
	}
...
func TestRealHello_PrintsHelloWorld(t *testing.T) {
	...
	inst, err := component.Instantiate(ctx, r, realHelloWasm, WithWASI(WASIConfig{Stdout: &stdout, Stderr: &stderr})...)
	if err != nil {
		t.Fatalf("Instantiate: %v", err)
	}
	defer inst.Close(ctx)
...
func TestRealHello_ReinstantiateAfterCloseOnSameRuntime(t *testing.T) {
	...
	inst1, err := component.Instantiate(ctx, r, realHelloWasm)
	if err != nil {
	...
	inst2, err := component.Instantiate(ctx, r, realHelloWasm)
	if err != nil {
