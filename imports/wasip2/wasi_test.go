...
// wasiHostFunc extracts the raw component.HostFunc WithWASI registered for iface/name,
...
// in isolation.
func wasiHostFunc(t *testing.T, cfg WASIConfig, iface, name string) component.HostFunc {
	t.Helper()
	// componenttest.New runs the resource hooks exactly as a real Instantiate
	// does before any host func can execute, so the filesystem funcs
	// (get-directories, open-at, read-via-stream, ...) have a live handle
	// table to mint own<T> handles through -- without a guest in the picture.
