...
func TestRealHTTPTrailers(t *testing.T) {
	...
	inst, err := component.Instantiate(ctx, r, realHTTPTrailersWasm, WithWASI(WASIConfig{EnableHTTP: true})...)
	if err != nil {
...
func TestRealHTTPRequestTrailers(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			inst, err := component.Instantiate(ctx, r, realHTTPReqTrailersWasm, WithWASI(WASIConfig{EnableHTTP: true})...)
			if err != nil {
				t.Fatalf("Instantiate: %v", err)
			}
			defer inst.Close(ctx)
