...
func Example_newHostLoggingListenerFactory() {
	...
	ctx := api.WithFunctionListenerFactory(context.Background(),
	...
	mod, err := r.InstantiateWithConfig(ctx, listenerWasm,
		wazy.NewModuleConfig().WithStdout(os.Stdout))
	if err != nil {
		log.Panicln(err)
	}
...
func Example_newLoggingListenerFactory() {
	...
	ctx := api.WithFunctionListenerFactory(context.Background(), logging.NewLoggingListenerFactory(os.Stdout))
