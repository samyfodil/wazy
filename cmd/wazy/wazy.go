...
func doMain(stdOut io.Writer, stdErr logging.Writer) int {
	...
	case "version":
		fmt.Fprintln(stdOut, version.GetWazyVersion())
		return 0
	default:
...
func doCompile(args []string, stdErr io.Writer) int {
	...
	if version.GetWazyVersion() != version.Default {
		count = 1
	} else {
		flags.IntVar(&count, "count", 1,
			"Number of times to perform the compilation. This is useful to benchmark performance of the wazy compiler.")
...
func doRun(args []string, stdOut io.Writer, stdErr logging.Writer) int {
	...
	flags.Var(&mounts, "mount",
		"Filesystem path to expose to the binary in the form of <path>[:<wasm path>][:ro]. "+
			...
			"If that is not desired, use wazy as a library and implement a custom fs.FS.")
	...
	var rtc wazy.RuntimeConfig
	if useInterpreter {
		rtc = wazy.NewRuntimeConfigInterpreter()
	} else {
		rtc = wazy.NewRuntimeConfig()
	}
	...
	rt := wazy.NewRuntimeWithConfig(ctx, rtc)
	defer rt.Close(ctx)
...
func validateMounts(mounts sliceFlag, stdErr logging.Writer) (rc int, rootPath string, config wazy.FSConfig) {
...
func cacheDirFlag(flags *flag.FlagSet) *string {
	return flags.String("cachedir", "", "Writeable directory for native code compiled from wasm. "+
		"Contents are re-used for the same version of wazy.")
}
...
func maybeUseCacheDir(cacheDir *string, stdErr io.Writer) (int, wazy.CompilationCache) {
	if dir := *cacheDir; dir != "" {
		if cache, err := wazy.NewCompilationCacheWithDir(dir); err != nil {
...
func printUsage(stdErr io.Writer) {
	...
	fmt.Fprintln(stdErr, "  version\tDisplays the version of wazy CLI")
}
...
func printRunUsage(stdErr io.Writer, flags *flag.FlagSet) {
	fmt.Fprintln(stdErr, "wazy CLI")
	fmt.Fprintln(stdErr)
	fmt.Fprintln(stdErr, "Usage:\n  wazy run <options> <path to wasm file> [--] <wasm args>")
	fmt.Fprintln(stdErr)
	fmt.Fprintln(stdErr, "Options:")
	flags.PrintDefaults()
}
